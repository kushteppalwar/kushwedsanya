...
import { label } from "@/components/v13/styles";
    ...
    >
      <div className="mx-auto max-w-4xl">
        <Reveal className="text-center">
          <p className={label}>The itinerary</p>
          <h2 className="mt-3 font-serif text-[clamp(2rem,6vw,4rem)] leading-none text-(--jm-ink) text-balance">
            {title}
          </h2>
          {note && (
            <p className="mx-auto mt-4 max-w-md font-sans text-lg font-medium text-(--jm-muted) italic sm:text-xl">
              {note}
            </p>
          )}
        </Reveal>
                ...
                <li key={`${stop.group}-${stop.name}`} className="relative">
                  {newGroup && (
                    <Reveal className="relative mb-6 pl-14 md:mb-8 md:pl-0 md:text-center">
                      <span className="inline-block rounded-full border border-(--jm-accent)/50 bg-(--jm-bg) px-4 py-1.5 text-xs font-medium tracking-[0.3em] text-(--jm-accent) uppercase sm:text-[0.8rem]">
                        {stop.group}
                        {stop.date && ` · ${stop.date}`}
                      </span>
                    </Reveal>
                  ...
                  <div className="relative md:grid md:grid-cols-[minmax(0,1fr)_4rem_minmax(0,1fr)] md:items-center">
                    ...
                    >
                      <article className="rounded-2xl border border-(--jm-line) bg-(--jm-card) p-5 sm:p-6">
                        ...
                        <p className="mt-1.5 font-sans text-lg font-medium text-(--jm-muted) sm:text-xl">
                          {stop.hall && (
                        ...
                        {stop.attire && (
                          <dl
                            className={`mt-4 space-y-1.5 border-t border-dashed border-(--jm-line) pt-3 text-[1.05rem] leading-snug font-medium text-(--jm-muted) sm:text-lg ${left ? "md:[&>div]:flex-row-reverse" : ""}`}
                          >
                            {(
                              [
                              ...
                              .map(([who, value]) => (
                                <div key={who} className="flex gap-2">
                                  <dt className="shrink-0 text-[0.7rem] leading-[1.9] font-semibold tracking-[0.25em] text-(--jm-accent)/85 uppercase sm:leading-[2]">
                                    {who}
                                  </dt>
