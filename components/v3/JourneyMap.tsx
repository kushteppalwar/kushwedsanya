...
function contour(cx: number, cy: number, radius: number, seed: number) {
  const points = Array.from({ length: 36 }, (_, i) => {
    ...
    const wobble = 1 + 0.12 * Math.sin(angle * 3 + seed) + 0.07 * Math.cos(angle * 5 - seed);
...
function Pin({ point, stop, align }: { point: Point; stop: JourneyStop; align: "start" | "end" }) {
  ...
  return (
    <g>
      <circle cx={point.x} cy={point.y} r="14" className="pin-pulse" fill="var(--jm-accent)" />
      <circle cx={point.x} cy={point.y} r="9" fill="var(--jm-bg)" stroke="var(--jm-accent)" strokeWidth="3" />
...
export default function JourneyMap({ from, to, className = "" }: JourneyMapProps) {
  ...
  return (
    ...
    >
      ...
      <g transform={`translate(${route.mid.x} ${route.mid.y - 34})`} textAnchor="middle">
        <rect x="-78" y="-22" width="156" height="34" rx="17" fill="var(--jm-card)" stroke="var(--jm-line)" />
        <text y="2" fill="var(--jm-ink)" fontFamily="var(--font-sans)" fontSize="19" letterSpacing="2">
      ...
      <g fill="var(--jm-accent)" opacity="0.7">
        {[[-70, -70, 12], [-32, -110, 8], [-105, -30, 7]].map(([dx, dy, s]) => (
          <path
            key={`${dx}-${dy}`}
      ...
      <g transform={`translate(40 ${MAP_SIZE - 150})`}>
        <rect width="290" height="96" fill="var(--jm-card)" stroke="var(--jm-ink)" strokeWidth="1" />
        <rect x="6" y="6" width="278" height="84" fill="none" stroke="var(--jm-ink)" strokeWidth="0.6" />
        <text x="145" y="38" textAnchor="middle" fill="var(--jm-ink)" fontFamily="var(--font-serif)" fontSize="20" letterSpacing="3">
          TWO STATES
        </text>
        <text x="145" y="70" textAnchor="middle" fill="var(--jm-accent)" fontFamily="var(--font-script)" fontSize="30">
          one celebration
        </text>
      </g>

      <rect width={MAP_SIZE} height={MAP_SIZE} fill="url(#journey-vignette)" pointerEvents="none" />
    </svg>
  );
}
