...
const WORLD_ZOOM_PORTRAIT = 0.5;

const eyebrow =
  "text-[0.62rem] tracking-[0.32em] text-(--jm-accent) uppercase sm:text-[0.65rem] sm:tracking-[0.35em]";
const heading =
  "mt-2 font-serif text-2xl leading-tight text-(--jm-ink) sm:mt-3 sm:text-4xl";
const body =
  "mt-2 font-sans text-base leading-relaxed text-(--jm-muted) sm:mt-3 sm:text-lg";
...
export default function WorldJourney({
  ...
  const camera: AtlasCameraKeyframe[] = [
    ...
    ...worldPath.map((point, index) => ({
      ...
      // Biased toward Delhi so it stays in frame while the far origin enters from the edge.
      point: {
        x: point.x + (venue.x - point.x) * 0.55,
        y: point.y + (venue.y - point.y) * 0.55,
      },
      zoom: WORLD_ZOOM,
  ...
  const originList = (places: MapPlace[]) => (
    <ul className="mt-3 space-y-1 sm:mt-4 sm:space-y-1.5">
      {places.map((place) => (
        <li
          key={place.city}
          className="flex items-baseline justify-between gap-4 font-sans text-[0.95rem] sm:text-base"
        >
  ...
  const stops: AtlasStop[] = [
    {
      ...
      content: (
        <>
          ...
          {invitation && (
            <p className="mt-3 font-sans text-base leading-relaxed text-(--jm-muted) italic sm:mt-4 sm:text-lg">
              {invitation.intro} {invitation.request}
            </p>
          )}
          <p className="mt-4 text-[0.62rem] tracking-[0.3em] text-(--jm-muted) uppercase sm:mt-5">
